...
└──────┬─────────┬───────────┘
       │         │
       │         ├── Asaas: checkout, assinatura e webhooks
       │         │
       ▼         ▼
┌────────────┐  ┌─────────────────────┐
│ PostgreSQL │  │ Storage S3/iDrive  │
│ Prisma     │  │ PDFs e URLs signed │
└────────────┘  └─────────────────────┘
```
...
| Componentes            | Lucide React, React PDF Viewer                   |
| Backend                | Node.js, Express, TypeScript                     |
| Autenticação           | JWT + bcryptjs                                   |
| Persistência           | PostgreSQL + Prisma 5                            |
| Arquivos               | AWS SDK S3 para storage compatível com S3/iDrive |
| Pagamentos             | Asaas                                            |
| Contrato compartilhado | `@margem/types`                                  |
...
O projeto não fixa um provedor de deploy no código. Um deploy de produção deve, no mínimo:

1. Instalar dependências com a versão de pnpm compatível.
2. Configurar todas as variáveis de ambiente fora do repositório.
3. Executar `prisma generate` e `prisma migrate deploy`.
4. Executar `pnpm build`.
5. Iniciar a API com `pnpm --filter api start`.
6. Iniciar o frontend com `pnpm --filter web start`.
7. Configurar `APP_URL`, `NEXT_PUBLIC_API_URL` e URLs públicas do Asaas.
8. Garantir que o webhook do Asaas alcance `POST /billing/webhooks/asaas`.
9. Validar `GET /health` e `GET /docs` após a publicação.

Para ambientes de produção, use um `JWT_SECRET` forte, credenciais de storage com escopo mínimo e `ASAAS_ENVIRONMENT=production` somente após validar o fluxo no sandbox.

## Segurança e arquivos locais

- Arquivos `.env` e `.env.local` são ignorados pelo Git.
- Apenas `.env.example`, sem credenciais reais, deve ser versionado.
- Não coloque tokens Asaas, chaves S3, senhas ou strings de conexão em commits.
- Os diretórios `node_modules`, `dist`, `.next`, caches e temporários também são ignorados.
- Em produção, não dependa dos valores fallback de desenvolvimento, especialmente para `JWT_SECRET`.
